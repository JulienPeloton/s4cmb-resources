...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        sky_out = partial2full(sky_out_tot.get_I(),
                               ...
                               fill_with=0.0)
        mask = nhit >= 1
        assert np.all(np.abs(sky_in.I[mask] - sky_out[mask]) < 1e-8), \
            ValueError("Output not equal to input in temperature!")

        sky_out = partial2full(sky_out_tot.get_QU()[0],
                               sky_out_tot.obspix, sky_out_tot.nside,
                               fill_with=0.0)
        mask = nhit >= 10
        assert np.all(np.abs(sky_in.Q[mask] - sky_out[mask]) < 1e-8), \
            ValueError("Output not equal to input for Stokes Q!")

        sky_out = partial2full(sky_out_tot.get_QU()[1],
                               sky_out_tot.obspix, sky_out_tot.nside,
                               fill_with=0.0)
        mask = nhit >= 10
        assert np.all(np.abs(sky_in.U[mask] - sky_out[mask]) < 1e-8), \
            ValueError("Output not equal to input for Stokes U!")
